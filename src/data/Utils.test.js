...
import { releasesForProduct, outlierProductUrls } from "./Utils.js";
...
Previous Name,Pants,Clothes,Strongtwill,2018-04-03,"Artificial Grape, Black Rock, Gray Rock, Lemon, Orange, Sky Blue",59,TRUE,FALSE,Unisex,http://shop.outlier.cc/shop/retail/grid-linen-towel.html,https://outlier.nyc/productentries/products/gridlinentowel/204-Outlier-GridLinenTowel-artificialgrape.jpg,,,,,http://archive-url
...
describe("releases", () => {
  ...
  it("Computes Releases for Latest Name", () => {
    ...
    return Data.read().then((releases) => {
      const { filteredReleases, numberOfReleaseNames } = releasesForProduct(
        releases,
        "Latest Name"
      );
...
describe("urls", () => {
  ...
  it("Computes URLs for filtered releases", () => {
    fetch.mockResponseOnce(simpleData);
    return Data.read().then((releases) => {
      const { filteredReleases } = releasesForProduct(releases, "Latest Name");
      const { outlierCcUrl, outlierNycUrl, archiveUrl } =
        outlierProductUrls(filteredReleases);
      expect(outlierCcUrl).toEqual(null);
      expect(outlierNycUrl).toEqual(
        "https://shop.outlier.nyc/shop/retail/strong-dungarees.html"
      );
      expect(archiveUrl).toEqual("http://archive-url");
    });
  });
});
