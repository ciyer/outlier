...
describe("state manipulation", () => {
  ...
  it("initializes as expected", () => {
    ...
    expect(state.archive.settings.display).toEqual({
      showImages: true,
      listing: "chronological",
    });
    expect(state.archive.settings.filters).toEqual({});
    expect(new FilterSummary(state.archive.settings.filters).compute()).toEqual(
      "(show all)"
    );
...
describe("data retrieval", () => {
  ...
  it("invokes expected actions", () => {
    ...
    const expectedActions = [
      ...
      {
        payload: { archiveData: simpleResponse },
        type: "ArchiveState.CONTENT_RETURN",
      },
    ];
    return store
      .dispatch(ArchiveState.data.retrieve("archiveData"))
      .then(() => {
...
describe("store update", () => {
  ...
  it("retrieves and filters data", () => {
    fetch.mockResponseOnce(simpleData);
    const store = createGlobalStore();
    const fullDataLength = 12;
    const baselinePriceBins = [59, 120, 125, 130, 185.4, 495];
    return store
      .dispatch(ArchiveState.data.retrieve("archiveData"))
      .then(() => {
        // Check that initial data is correct
        expect(store.getState().archive.data.full.length).toEqual(
          fullDataLength
        );
        expect(store.getState().archive.data.filtered.length).toEqual(
          fullDataLength
        );
        let filters = store.getState().archive.settings.filters;
        expect(Object.keys(filters).length).toEqual(8);
        expect(store.getState().archive.summary.baseline.priceBins).toEqual(
          baselinePriceBins
        );
        expect(store.getState().archive.settings.display).toEqual({
          showImages: true,
          listing: "chronological",
        });
        ...
        const pantsFilter =
          store.getState().archive.settings.filters.Category[2];
        ...
        expect(
          store.getState().archive.settings.filters.Category[2].isOn
        ).toEqual(true);
        expect(store.getState().archive.data.full.length).toEqual(
          fullDataLength
        );
        expect(store.getState().archive.data.filtered.length).toEqual(7);
        expect(store.getState().archive.summary.baseline.priceBins).toEqual(
          baselinePriceBins
        );
        expect(store.getState().archive.settings.display).toEqual({
        ...
        expect(
          new FilterSummary(store.getState().archive.settings.filters).compute()
        ).toEqual("Pants");

        // Add another filter
        const springFilter =
          store.getState().archive.settings.filters.Season[1];
        ...
        expect(
          store.getState().archive.settings.filters.Season[1].isOn
        ).toEqual(true);
        expect(store.getState().archive.data.filtered.length).toEqual(7);
        expect(
          new FilterSummary(store.getState().archive.settings.filters).compute()
        ).toEqual("Pants and Spring");

        // Add another filter
        const outerwearFilter =
          store.getState().archive.settings.filters.Category[1];
        ...
        expect(
          store.getState().archive.settings.filters.Category[1].isOn
        ).toEqual(true);
        expect(store.getState().archive.data.filtered.length).toEqual(8);
        expect(
          new FilterSummary(store.getState().archive.settings.filters).compute()
        ).toEqual("(Outerwear or Pants) and Spring");

        // Add yet another filter
        const shortsFilter =
          store.getState().archive.settings.filters.GarmentTypes[2];
        ...
        expect(
          store.getState().archive.settings.filters.GarmentTypes[2].isOn
        ).toEqual(true);
        expect(store.getState().archive.data.filtered.length).toEqual(5);
        expect(
          new FilterSummary(store.getState().archive.settings.filters).compute()
        ).toEqual("(Outerwear or Pants) and Shorts and Spring");
