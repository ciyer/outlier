...
describe('page retrieval', () => {
  ...
  it('invokes expected actions', () => {
    ...
    const expectedActions = [
      ...
      {"payload": {"simplePageData": simplePageData}, "type": "MarkdownPageState.CONTENT_RETURN"}];
    return store.dispatch(MarkdownPageState.retrieve('simplePageData')).then(() => {
  ...
  it('updates store', () => {
    ...
    store.dispatch(MarkdownPageState.retrieve('simplePageData')).then(() => {
        // return of async actions
        expect(store.getState().pages).toEqual({"simplePageData": simplePageData})
      })
  });
});
